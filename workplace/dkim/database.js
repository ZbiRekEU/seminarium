...
class database {
  ...
  async updateOrderData(data) {
    return new Promise((resolve, reject) => {
      pool.query('UPDATE orders SET customer_name = ?, city = ?, postal_code = ?, street = ?, customer_phone = ?, house_number = ?, apartment_number = ?, products = ?,  additional_info = ?, price = ?, status = ?, status_details = ?, payment = ?, planned_date = ?, planned_time = ?, settled = ?, SMS_from_company = ?, EMAIL_from_company = ?, SMS_from_client = ?, EMAIL_from_client = ?, email_client = ?, email_company = ?, sms_client = ?, sms_company = ? WHERE order_id LIKE ?', [data.customer_name, data.city, data.postal_code, data.street, data.customer_phone, data.house_number, data.apartment_number, data.products, data.additional_info, data.price, data.status, data.status_details, data.payment, data.planned_date, data.planned_time, data.settled, data.SMS_from_company, data.EMAIL_from_company, data.SMS_from_client, data.EMAIL_from_client, data.email_client, data.email_company, data.sms_client, data.sms_company, data.order_id], function (error, results, fields) {
        if (error) throw error;
        var res = JSON.parse(JSON.stringify(results))
        // console.log(res);
        if (res.changedRows > 0) resolve(res)
